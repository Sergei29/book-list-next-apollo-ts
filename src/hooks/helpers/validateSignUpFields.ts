...
const validatePassword = (strPassword: string): ObjValidationType => ({
  ...
  strErrorMessage: strPassword.length >= 8 ? '' : 'must be at least 8 characters long',
})

const validateConfirmPassword = (strPassword: string, strConfirmPassword: string): ObjValidationType => ({
  bIsValid: strPassword === strConfirmPassword,
  strErrorMessage: strPassword === strConfirmPassword ? '' : 'Passwords not matching',
})
...
export const funcIsFormValid = (
  ...
  Object.values(objFormValidation).reduce(
    (bFormValid, objCurrentValidation) => bFormValid && objCurrentValidation.bIsValid,
    true,
  ) &&
